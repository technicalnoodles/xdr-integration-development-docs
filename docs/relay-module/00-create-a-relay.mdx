...
To test the module from our on-prem device (laptop, desktop or other internal development server), we will use ngrok to faciliate the connection between the local server and XDR.
...
Then follow the isntructions to get a ngrok auth token added to your profile. Then using the _Ephemeral Domain_ option, run the command listed but change the port to 5000. It should look similar to this command `ngrok http http://localhost:5000`.
...
For this guide, we will use the `crt.sh` service as the our integration. You may use any that want, but to be able to follow along step by step, we recommend using the `crt.sh` service as well.

In the _Integration Name_ field, input _CRT.SH_.
...
![relay settings](../../static/img/relay/settings.png)
...
![relay settings](../../static/img/relay/generic-relay-connected.png)

With this, we have finished making sure the module is running and accessible and that XDR can connect to it.

:::note
In the future, we will need to move away from using this generic module and move towards a full module which has a _module definition_ added to XDR. We will keep that in mind moving forward and make it fairly easy to make that conversion later on.
:::